...
test('ツールチップ - 複数要素の連続ホバー', async ({ page }) => {
  ...
  const hoverButton = page.locator('#toolTipButton'); //ホバーボタンを取得
  const textField = page.locator('#toolTipTextField'); //テキストフィールドを取得
  const textFieldToolTip = page.locator('#textFieldToolTip'); //テキストフィールドのツールチップを取得
  const buttonToolTip = page.locator('#buttonToolTip'); //ツールチップ要素を取得

  // 最初の要素にホバー
  await hoverButton.hover(); //ボタンにマウスホバー
  await expect(buttonToolTip).toContainText('You hovered over the Button'); //ボタンのツールチップを確認
  ...
  // 2番目の要素にホバー
  await textField.hover(); //テキストフィールドにマウスホバー
  await expect(textFieldToolTip).toContainText('You hovered over the text field'); //テキストフィールドのツールチップを確認

  // ツールチップの内容が正しく切り替わったことを確認
  await expect(buttonToolTip).not.toBeVisible(); //前のツールチップテキストが残っていないことを確認
});
